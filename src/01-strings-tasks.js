...
function encodeToRot13(str) {
  const firstArr = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'.split(
    ''
  );
  const secondArr =
  ...
  const result = str.split('').map(function map(el) {
    if (firstArr.includes(el)) {
      index = firstArr.indexOf(el);
      return secondArr[index];
    }
    if (secondArr.includes(el)) {
      index = secondArr.indexOf(el);
